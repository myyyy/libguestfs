                  ...
                  void (*notify_ui) (int type, const char *data))
{
  ...
  CLEANUP_FREE char *remote_dir = NULL, *libvirt_xml = NULL, *config_xml = NULL;
  time_t now;
  struct tm tm;
...
#if DEBUG_STDERR
  fprintf (stderr, "%s: config XML:\n%s", guestfs_int_program_name, config_xml);
#endif
  }
  ...
  control_h = start_remote_connection (config, remote_dir, libvirt_xml, config_xml);
  if (control_h == NULL) {
    const char *err = get_ssh_error ();
...
generate_config_xml (struct config *config, struct data_conn *data_conns)
{
  ...
  comment
    (" NOTE!\n"
     "\n"
     "  This config XML is generated by the virt-p2v front end, in\n"
     "  order to communicate with the backend virt-v2v process running\n"
     "  on the conversion server. It is a description of the mapping\n"
     "  between physical disks, netcards and EverRun storage groups,\n"
     "  virtual networks. ");


  start_element ("configs") {

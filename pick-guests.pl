#!/usr/bin/perl -w
# pick-guests.pl.  Generated from pick-guests.pl.in by configure.
...
my $vmm = Sys::Virt->new (uri => 'qemu+unix:///system?socket=/var/run/libvirt/libvirt-sock-ro');
...
foreach my $dom (@domains) {
    ...
    eval {
        $g->add_domain ($name, readonly => 1,
                        libvirturi => 'qemu+unix:///system?socket=/var/run/libvirt/libvirt-sock-ro');
